...
public:
  MotionPlanningServer()
    : Node("motion_planning_server_node")
  {
...
private:
                   ...
                   std::shared_ptr<crs_msgs::srv::PlanProcessMotions::Response> response)
  {
      ...
      if (request->start_position.position.size() > 0)
      {
        ...
        motion_planner_config_->start_pose = std::make_shared<tesseract_motion_planners::JointWaypoint>(request->start_position.position, request->start_position.name);
      }
      if (request->end_position.position.size() > 0)
      {
        motion_planner_config_->use_end = true;
        motion_planner_config_->end_pose = std::make_shared<tesseract_motion_planners::JointWaypoint>(request->end_position.position, request->end_position.name);
      }
      ...
      for (size_t i = 0; i < request->process_paths.size(); ++i)
      {
          ...
          crs_motion_planning::rasterStripsToMarkerArray(motion_planner_config_->rasters, "world", mark_array_msg, {1.0, 0.0, 0.0, 1.0}, -0.01);
          ...
          crs_motion_planning::rasterStripsToMarkerArray(path_plan_results->solved_rasters, motion_planner_config_->world_frame, temp_mark_array_msg, {1.0, 0.0, 1.0, 0.0}, -0.025);
          crs_motion_planning::rasterStripsToMarkerArray(path_plan_results->failed_rasters, motion_planner_config_->world_frame, temp_mark_array_msg, {1.0, 1.0, 0.0, 0.0}, -0.025);
          crs_motion_planning::rasterStripsToMarkerArray(path_plan_results->skipped_rasters, motion_planner_config_->world_frame, temp_mark_array_msg, {1.0, 1.0, 1.0, 0.0}, -0.025);
          ...
          crs_motion_planning::failedEdgesToMarkerArray(path_plan_results->unreachable_waypoints, motion_planner_config_->world_frame, failed_vertex_markers, {1.0, 0.0, 1.0, 1.0}, 0.01);
                     ...
                     std::shared_ptr<crs_msgs::srv::CallFreespaceMotion::Response> response)
  {
      ...
      {
          motion_planner_config_->ompl_config.n_output_states = this->get_parameter(param_names::NUM_FREEPSACE_STEPS).as_int();
      }
      ...
      if(request->goal_position.position.empty())
      {
        ...
        success = crs_motion_planner.generateFreespacePlan(joint_start_waypoint, goal_waypoint, response->output_trajectory);
      }
      else
      {
